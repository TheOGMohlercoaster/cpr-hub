...
const ROUTES = [
  { match: 'purchaseorder', tab: 'PurchaseOrders' },
  { match: 'purchase',      tab: 'PurchaseOrders' },
  { match: 'lead',          tab: 'Leads' },
  { match: 'devicesales',   tab: 'DeviceSales' },
  { match: 'device',        tab: 'DeviceSales' },
  { match: 'repairsales',   tab: 'RepairSales' },
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Allow an explicit override via query string: ?tab=PurchaseOrders
    const override = req.query.tab;
    const route = override
      ? { tab: override }
      : ROUTES.find((r) => norm(title).includes(norm(r.match)));
    ...
    return res.status(200).json({
      ...
      title,
      tab: route.tab,
      matchedBy: override ? 'query override' : 'title',
      rows: rows.length - 1,
